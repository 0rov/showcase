...
/**
 * @brief  读取ILI9341的某点的颜色值
 * @param  x,y:坐标
 * @retval 此点的颜色
 */
uint16_t ILI9341_LCD_ReadPoint ( uint16_t usX, uint16_t usY )	
{	
	uint16_t usR=0, usG=0, usB=0 ;

	if ( ( usX >= LCD_X_LENGTH ) || ( usY >= LCD_Y_LENGTH ) )return 0;
	ILI9341_SetCursor ( usX, usY );	
	ILI9341_Write_Cmd ( 0x2E );   /* 读数据 */
 ...
 * @retval 无
 */
void ILI9341_DrawTrapezium ( uint16_t usX_Center, uint16_t usY_Center, uint16_t usWidth, uint16_t usHeight)
{
	uint16_t hmax, wmax, hmin, wmin;
	hmax = usY_Center + usHeight/2;
	wmax = usX_Center + usWidth/2 + usHeight/3;
	hmin = usY_Center - usHeight/2;
	wmin = usX_Center - usWidth/2 - usHeight/3;
	if(wmax >=320)
		wmax = 319;
	if(hmax >=240)
	  wmax = 239;		
		ILI9341_DrawLine ( usX_Center - usWidth/2 + usHeight/3, hmin, usX_Center + usWidth/2 - usHeight/3, hmin );
		ILI9341_DrawLine ( wmin, hmax, wmax, hmax );
		ILI9341_DrawLine ( wmin, hmax, usX_Center - usWidth/2 + usHeight/3, hmin );
		ILI9341_DrawLine ( wmax, hmax, usX_Center + usWidth/2 - usHeight/3, hmin );		
}
