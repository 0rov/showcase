...
****************************************************/
uint8_t Itera_Threshold(uint16_t PIXEL_W,uint16_t PIXEL_H)
{
  ...
  u16 threshold_h[256];
	...
	*/
	...
	/*
	//在液晶屏上面写入图像分割后的图像
  ILI9341_OpenWindow(0,0,PIXEL_W,PIXEL_H);
	ILI9341_Write_Cmd ( CMD_SetPixel );	
	//根据最佳阈值将图像二值化
	for( i=0; i<320; i++ )
	{
...
/**
  ...
  * @retval 无
  */
void ImagDisp_BW(uint16_t sx,uint16_t sy,uint16_t width,uint16_t height,uint8_t Itera_Threshold)
{
	uint16_t i, j; 
	uint16_t Camera_Data;
	uint16_t R,G,B,mux;
